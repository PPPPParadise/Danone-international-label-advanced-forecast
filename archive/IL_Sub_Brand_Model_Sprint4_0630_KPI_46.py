...
config["result_folder_path"] = "sub_brand/without_Q2"
...
#temp files
config["feature_import_first_run"] = "feature_importance_df_sets_0624_xg.csv"
config["version_name"] = 'blending_with_Q2_sprint4_all_feature_0624_xg'

# Parameter configuration
config["train_start"] = 201601
config["train_end"] = 201912
config["backtest_start"] = 201712
...
#     'ANZ','DE','NL','UK',
#     'APT','KC','NC','C&G',
#     'ANZ_APT_PF', 'ANZ_APT_PN', 'ANZ_KC_GD', 'ANZ_KC_GT', 'DE_APT_PF',
#     'DE_APT_PN', 'NL_NC_PN', 'UK_APT_PF', 'UK_APT_PN', 'UK_C&G_C&G',
    'horizon',
    'C&G', 'GD','GT', 'PF', 'PN', 
...
#         sales_danone = self.sales_danone.copy()
    ...
    all_sales = all_sales[
        ['calendar_yearmonth', 'country_brand', 'country', 'brand','tier','sub_brand','label', 'offtake',
         'sellin','retailer_inv','sp_inv','sellout','price','upre','spre','mainstream',
         'anp', 'tp', 'rebate', 'tp_rebate', 'sc_anp','sc_ts']]
    return all_sales 
...
#     grouped_offtake2 = df.groupby(['calendar_yearmonth', 'label','tier'])[
#                     'offtake'].sum().unstack('calendar_yearmonth').fillna(0)
...
#                     'offtake'].sum().unstack('calendar_yearmonth').fillna(0)
#     df6 = features_amount_sales(grouped_offtake2, dwp, dtp, 'country_label_offtake_')  

    grouped_offtake2 = df.groupby(['calendar_yearmonth','label','country','brand', 'tier','country_brand','sub_brand'])[
                    'sellin'].sum().unstack('calendar_yearmonth').fillna(0)
    df7 = features_amount_sales(grouped_offtake2, dwp, dtp, 'label_country_sub_brand_sellin_') 
...
def model_single_run(config,dwp_test,table_all_features,df):
    ...
    features = [x for x in table_all_features.keys() if (x not in config["features_int"])&(x in config["feature_test"])]
...
def output_feature_importance(config,feature_importance_df):
    ...
    for h in range(1,13):
        ...
        sort_values(['importance'], ascending = False)[:40]['index'].values
...
def calculate_KPI(df_il,config):
    ...
    ytd_forecasts_kpis_2019.to_csv(config["project_folder_path"] + '/' +\
                                   config["result_folder_path"] + '/' + 'kpi_' +\
                                   config["version_name"] + '.csv',index = False)         
    df_kpi = ytd_forecasts_kpis_2019.copy() 
    df_kpi['version'] = config["version_name"]
    kpis_with_Q2 = kpi_stats.KPI_formalization(df_kpi,kpi_name = 'bias',skip_month = '2019-06', version = config["version_name"]) 
    kpis_with_Q2.generate_KPI()
...
def output_res_kpi(df_il,config):
    ...
    df_il.to_csv(config["project_folder_path"] + '/' +\
                 config["result_folder_path"] + '/' + 'res_' +\
                 config["version_name"] + '.csv',index = False)
    calculate_KPI(df_il,config)
...
#         feature_importance_df_sets = Load_50_feature(config)            ## 1.3 Load rank firt 50 features
    ...
    table_all_features = Merge_table_with_category(table_all_features,
                                                   df_feature_addon)      ## 2.7 Merge rolling features
    ...
    # 4.Calculate KPI
    df_il = Get_actual(resfinal,df_)                                       ## 4.1 Merge with raw master to get acutal offtake

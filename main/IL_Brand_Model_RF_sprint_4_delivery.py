...
config["result_folder_path"] = "../sprint4"
...
#             model = grid_search.best_estimator_
            print (model)
            Pkl_Filename = config['model'] + '/' + "RF_Model_{0}_{1}.pkl".format(datwep,h) 
...
def calculate_KPI(df_il,name):
    ...
    df_kpi.to_csv(config["result_folder_path"] + '/' +\
                  'kpi_0631_withou_Q2_{0}.csv'.format(name),index = False)
    df_kpi['version'] = name
...
def output_res_kpi(df_il,config):
    ...
    df_il.to_csv(config["result_folder_path"] + '/' +\
                 'res_0631_without_Q2_{0}.csv'.format('RF'),index = False)  
...
def output_feature_importance(config,feature_importance_df):
    ...
    if config["FirstRun"]:
        feature_importance_df_sets_.to_csv(config['temp_folder_path'] +\

...
class KPI_formalization:
    ...
    def calculate_brand_ytd(self):
        ...
        brand_ytd = pd.pivot_table(self.KPI.query("level == '%s'" %'country_brand').\
                                            query("kpi_type == '%s'" %'ytd').\
                                            query("version == '%s'" %self.version).\
                                            query("month == '%s'" %'2019-12'),
                                   index = ['horizon','computed_on'],
                                   columns = ['month'],
                                   values = ['value'])
    ...
    def calculate_ttl_ytd(self):
        ...
        ttl_ytd = pd.pivot_table(self.KPI.query("level == '%s'" %'all_skus').\
                                     query("kpi_type == '%s'" %'ytd').\
                                     query("version == '%s'" %self.version).\
                                     query("month == '%s'" %'2019-12'),
                                 index = ['horizon','computed_on'],
    ...
    def calculate_ttl_month(self):
        ...
        ttl_month = pd.pivot_table(self.KPI.\
                                        query("level == '%s'" %'all_skus').\
                                        query("kpi_type == '%s'" %'month').\
                                        query("version == '%s'" %self.version),
                                   index = ['horizon','computed_on'],
                                   columns = ['month'],
                                   values = ['value'])

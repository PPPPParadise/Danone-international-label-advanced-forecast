...
class Modelil(Model):
    ...
    def preformat_table(self):
        ...
        all_sales = all_sales[
            ['calendar_yearmonth', 'sku_wo_pkg', 'country', 'brand', 'tier',
             'stage', 'label', 'offtake', 'sellin', 'sellout','index_cn','index_en']]
    ...
    def create_all_features(self, dwp, dtp):
        ...
        # 17. index_cn
        grouped_offtake2 = self.all_sales.drop_duplicates(subset=['calendar_yearmonth'])
        grouped_offtake2 = pd.DataFrame(grouped_offtake2['index_cn'].values, index=grouped_offtake2['calendar_yearmonth'])
        grouped_offtake2 = grouped_offtake2.T
        df17 = features_amount_sales(grouped_offtake2, dwp, dtp, 'index_cn', timelag=3)
        ##18. index_en
        grouped_offtake2 = self.all_sales.drop_duplicates(subset=['calendar_yearmonth'])
        grouped_offtake2 = pd.DataFrame(grouped_offtake2['index_en'].values, index=grouped_offtake2['calendar_yearmonth'])
        grouped_offtake2 = grouped_offtake2.T
        df18 = features_amount_sales(grouped_offtake2, dwp, dtp, 'index_en', timelag=3)


        dfs = [df1, df2, df3, df4, df5, df6, df7, df8, df9, df10, df11, df12, df13, df14, df15, df16,df17,df18,target]
        ...
        for datwep in list_dwps:
            ...
            for h in range(1, horizon + 1):
                ...
                if not self.debug:
                    ...
                    filename = f"{h}_XGB_model_forecast.pickle"
    ...
    def recreate_past_forecasts_di(self, table_all_features, list_dwps, params,
                                max_available_date, horizon=10):
...
if __name__ == '__main__':
    ...
    raw_master = pd.read_csv('./data/raw/raw_master_il_1019_searchindex.csv')
    ...
    dwp_test = util.create_list_period(201711, 201908, False)
    ...
    test.cvr.to_csv('./data/cvr_1019_searchindex.csv')
    temp = test.run_test(plot=False, prediction_horizon=3)


    # 3. run model
    # res = mod.recreate_past_forecasts(table_all_features, dwp_test, dico_params, filter_date=filter_date)

...
from cfg.paths import DIR_MAPPINGS, DIR_CACHE, DIR_DATA, DIR_CFG, DIR_EXPORT_QUALITY_CHECKS, RUN_DATETIME
...
today = date.today()
if today.day > 25:
    C_CYCLE_MONTH_LAG_IL = 1
else:
    C_CYCLE_MONTH_LAG_IL = 0
...
if C_CYCLE_MONTH_LAG_IL not in [-1, 0, 1]:
...
class DataPrep:
    ...
    def prep_di_tradeflow(self, file='di_tradeflow', bcg_sku_mapping='bcg_sku_mapping',
        ...
        today = date.today()
        m = 1 - C_CYCLE_MONTH_LAG_IL
        date_m_1 = datetime(today.year, today.month, 1)
        date_m_1 = date_m_1 - relativedelta(months=m)
        date_m_2 = datetime(today.year, today.month, 1)
        date_m_2 = date_m_2 - relativedelta(months=C_IL_RANGE - C_CYCLE_MONTH_LAG_IL)
        date_m_4_plan = datetime(today.year, today.month, 1)
        date_m_4_plan = date_m_4_plan + relativedelta(months=m + 3)
        date_m_3_plan = datetime(today.year, today.month, 1)
        date_m_3_plan = date_m_3_plan + relativedelta(months=m + 2)
        ...
        df = df.rename(columns={'quantity_ton': 'quantity',
                                'SKU_std': 'sku_code'})
        # todo: filter out all rows corresponding to sellin and date < latest
    ...
    def prep_il_sellin(self, file='il_sellin'):
        ...
        """
        today = date.today()
        date_m_2 = datetime(today.year, today.month - (C_IL_RANGE - C_CYCLE_MONTH_LAG_IL), 1)
        ...
        df_eib = df_eib.drop_duplicates()
        df_eib = df_eib.groupby(['sku_code', 'date', 'status']).sum().reset_index()  # remove dup with groupby+sum
        ...
        df_il_sellin['DI'] = df_il_sellin['DI'].apply(lambda x: round(x, 4))
        df_il_sellin['EIB'] = df_il_sellin['EIB'].apply(lambda x: round(x, 4))
        ...
        df_il_sellin['value'] = df_il_sellin['value'].apply(lambda x: round(x, 4))
    ...
    def prep_unified_di(self,
        ...
        df_sku_mapping = df_sku_mapping.loc[:, ['SKU', 'Country_acc', 'Brand_acc', 'Tier_acc',
                                                'Stage_acc', 'Package_acc', 'SKU_wo_pkg']]
    ...
    def create_quality_check(self, df_to_check: pd.DataFrame, hdfs_filename: str) -> None:
        ...
        name, ext = hdfs_filename.split('.')
        name_parts = name.split('_')
        timestamp = RUN_DATETIME.strftime("%Y%m%d%H%M%S")
        qc_df.to_csv(
            os.path.join(
                DIR_EXPORT_QUALITY_CHECKS,
                f"{timestamp}_qualitycheck_{'_'.join(name_parts[:-1])}_output.csv"
            )
        )

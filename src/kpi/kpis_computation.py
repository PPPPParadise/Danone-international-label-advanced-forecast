...
def bias(k, m, data, scope, agg_level="sku", verbose=False):
    ...
    data.loc[data['actual'] == 0, 'error'] = 0  # to ensure proper KPI computation
...
def YTD_bias(k, m, data, scope, agg_level="sku", first_month=None, verbose=False):
    ...
    for forecasted_month in list_forecasted_months: # For each forecasted month:
        ...
        if len(data_2) == 0:
            if verbose:
                print("YTD Bias M-%s from month %s, at month %s: No data for the scope %s, "
                      "computation month %s and forecasted month %s."
                      % (k, first_month, m, scope, comp_date, forecasted_month))
            return None
        
        # Compute the sum (over SKUs) of fcst and errors for computation_month fcst, using the fcst of month comp_date
        errors.append(
            sum(data_2.loc[data_2['actual'] > 0, "actual"].values - data_2.loc[data_2['actual'] > 0, "forecast"].values)
        )
...
def FA(k, m, data, scope, agg_level="sku", verbose=False):
    ...
    if len(data) == 0:
        if verbose:
            print("FA M-%s at month %s: No data for the scope %s, computation month %s and forecasted month %s."
                  % (k, m, scope, comp_date, m))
        return None

    data["abs_error"] = data.apply(lambda row: abs(row["actual"] - row["forecast"] if row['actual'] > 0 else 0), axis=1)
...
def YTD_FA(k, m, data, scope, agg_level="sku", first_month=None, verbose=False):
    ...
    for forecasted_month in list_forecasted_months: # For each forecasted month:
        ...
        if len(data_2) == 0:
            if verbose:
                print("YTD FA M-%s at month %s: No data for the scope %s, computation month %s and forecasted month %s."
                      % (k, first_month, scope, comp_date, forecasted_month))
            return None
        
        # Compute the sum (over SKUs) of fcst and errors for computation_month fcst, using the fcst of month comp_date
        data_2["abs_error"] = data_2.apply(lambda row: abs(row["actual"] - row["forecast"]) if row['actual'] > 0 else 0, axis=1)
...
def rolling_bias(r, k, m, data, scope, agg_level="sku", verbose=0):
    ...
    for i in range(r):
        ...
        data_2.loc[data_2['actual'] == 0, 'error'] = 0  # to ensure proper KPI computation
...
def YTD_rolling_bias(r, k, m, data, scope, agg_level="sku", first_month=None, verbose=0):
    ...
    for forecasted_month in list_forecasted_months: # For each forecasted month (for which rolling KPI computed)
        ...
        for i in range(r):
            ...
            data_3.loc[data_3['actual'] == 0, 'error'] = 0  # to ensure proper KPI computation
    ...
    if verbose:
        ...
        print("Computation details:")
        print("Rolling period, lag: %s, %s" %(r,k))
